...
regexes=(
    ".{75,}" # Ignore lines with more than 100 characters (overly specific)
    "[0-9]{4,}" # Ignore lines with 4 or more consecutive digits (likely an id)
    "[0-9]{4,}$" # Ignore lines where the last 3 or more characters are digits (likely an id)
    ...
    "\.(png|jpg|jpeg|gif|svg|bmp|ttf|avif|wav|mp4|aac|ajax|)$" # Ignore low value filetypes
)
